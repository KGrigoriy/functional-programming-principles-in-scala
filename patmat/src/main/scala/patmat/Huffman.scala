...
trait Huffman extends HuffmanInterface {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case x::xs => {
      ...
      xs.tail.filter(tree => weight(tree) < weight(newTree)):::
        newTree :: xs.tail.filter(tree => weight(tree) >= weight(newTree))
    }
    case _ => trees
  }
   ...
   */
  def until(done: List[CodeTree] => Boolean, merge: List[CodeTree] => List[CodeTree])(trees: List[CodeTree])
  : List[CodeTree] =
    done(trees) match {
    case true => trees
  ...
  val frenchCode: CodeTree = Fork(Fork(Fork(Leaf('s',121895),Fork(Leaf('d',56269),Fork(Fork(Fork(Leaf('x',5928)
    ...
    ,Fork(Leaf('c',50003),Fork(Leaf('v',24975),Fork(Leaf('g',13288),Leaf('b',13822),List('g','b'),27110)
      ,List('v','g','b'),52085),List('c','v','g','b'),102088),List('r','c','v','g','b'),202588),Fork(Leaf('n',108812)
    ,Leaf('t',111103),List('n','t'),219915),List('r','c','v','g','b','n','t'),422503),Fork(Leaf('e',225947)
    ,Fork(Leaf('i',115465),Leaf('a',117110),List('i','a'),232575),List('e','i','a'),458522)
    ,List('r','c','v','g','b','n','t','e','i','a'),881025)
    ,List('s','d','x','j','f','z','k','w','y','h','q','o','l','m','p','u','r','c','v','g','b','n','t','e','i','a')
    ,1486387)
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def encodeImpl(cTree: CodeTree, char: Char, acc: List[Bit]): List[Bit] = cTree match {
        case cTree: Leaf => acc
        case cTree: Fork =>
          if (chars(cTree.left) contains char) encodeImpl(cTree.left, char, acc :+ 0)
          else encodeImpl(cTree.right, char, acc :+ 1)
      }
